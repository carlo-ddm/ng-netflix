...
import { Genres, Obj } from '../interfaces/genres';
...
export class HttpService {
  ...
  genres_endpoint:string = 'https://api.themoviedb.org/3/genre/';
  query:string = 'movie/list?'
  ...
  httpGetGenres(api_key: string, language: string):Observable<Obj>{
    ...
    return this.http.get<Obj>(this.genres_endpoint + this.query, {params})
